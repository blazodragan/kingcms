        ...
        <Modal type="danger">
          ...
          <template #buttons="{ setIsOpen }">
            <Button
              @click.prevent="
                () => {
                  bulkAction('post', route('posts.bulk-destroy'), {
            ...
            <Modal type="danger">
              ...
              <template #buttons="{ setIsOpen }">
                <Button
                  @click.prevent="
                    () => {
                      action('delete', route('post.destroy', item), {
                        onFinish: () => setIsOpen(false),
                      });
                    }
                  "
...
const downloadFile = () => {
    ...
    if(url.length > 1) {
      window.location = route('posts.export', url.pop()).slice(0, -1);
    } else {
      window.location = route('posts.export');
    }
}
</script>
