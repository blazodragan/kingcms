...
@endif
        return redirect()->route('{{$modelRoute}}.index')->with(['message' => ___('craftable-pro', 'Operation successful')]);
    }
...
@endif
        if (session('{{$modelNamePluralLowerCase}}_url')) {
            return redirect(session('{{$modelNamePluralLowerCase}}_url'))->with(['message' => ___('craftable-pro', 'Operation successful')]);
        }
