...
@section('title', $post->title ?? 'Default Title')
...
<section>
    <div class="bg-[url('panoramic-aerial-shot-california-bixby-bridge-green-hill-near-beautiful-blue-water.jpg')] bg-no-repeat  bg-cover w-full p-5"> <!-- Hero Section -->
        <div class="w-full xl:w-1/2 mx-auto m-auto text-center flex"> <!-- Container Box -->
            <h1 class="text-2xl w-full text-white">{{__('Blog Post')}}</h1>
        </div>
    </div>
</section>
<section class="antialiased bg-white  p-10">
    <h1 class="text-4xl mb-10 text-site-blue-dark text-center">{{ $post->title }}</h1>
    ...
    <img class="absolute inset-0 w-full h-full object-cover transform hover:scale-105 transition duration-700 ease-out" src="{{ $post->media->first()->getUrl() }}" alt="{{ $post->media->first()->custom_properties['name'] ?? '' }}">
