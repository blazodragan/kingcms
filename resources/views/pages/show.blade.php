...
@section('title', $childPage->title ?? 'Default Title')
...
<section class="antialiased bg-white  p-10">
    <h1 class="text-4xl mb-10 text-site-blue-dark text-center">{{ $childPage->title }}</h1>
        ...
        <div class="w-full lg:w-2/6 p-4 ">
            <div class="rounded-lg overflow-hidden shadow">
                ...
                <div class="p-12 flex-col">
                    @if($childPage && $childPage->tips->count() > 0)
                    @foreach($childPage->tips->where('type', 'suggestion') as $tip)
                    <div>
...
@if($childPage && $childPage->tips->where('type', 'question')->count() > 0)
<section class="antialiased bg-site-bg-gray p-10">
    <div class="w-4/5 sm:w-full 2xl:w-3/5 mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        @foreach($childPage->tips->where('type', 'question') as $tip)
...
<section class="antialiased bg-gray-50  p-12">
    ...
    @if($childPage->media->first())
    <figure class="relative h-0 pb-[36.25%] overflow-hidden mb-8 rounded">
    <img class="absolute inset-0 w-full h-full object-cover transform hover:scale-105 transition duration-700 ease-out" src="{{ $childPage->media->first()->getUrl() }}" alt="{{ $childPage->media->first()->custom_properties['name'] ?? '' }}">
...
<section class="bg-site-bg-gray">
    <div class="w-3/4 lg:w-1/2 mx-auto">
        ...
        <div class="faq-box">
            @if($childPage && $childPage->faqs->count() > 0)
            @foreach($childPage->faqs as $faq)
